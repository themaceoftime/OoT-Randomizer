...
int16_t get_override_drop_id(int16_t dropId, uint16_t params) {
    ...
    if(((dropId == ITEM00_BOMBS_A) || (dropId == ITEM00_BOMBS_SPECIAL) || (dropId == ITEM00_BOMBS_B))) {
        if(BOMBCHUS_IN_LOGIC) {
            if((z64_file.items[Z64_SLOT_BOMB] == Z64_ITEM_BOMB) && (z64_file.items[Z64_SLOT_BOMBCHU] == Z64_ITEM_BOMBCHU)) { //we have bombs and chus
                return drop_bombs_or_chus(dropId);    
            } else if(z64_file.items[Z64_SLOT_BOMB] == Z64_ITEM_BOMB) { //only have bombs
                //don't do anything because this is already the right drop ID
                return dropId;
            } else if(z64_file.items[Z64_SLOT_BOMBCHU] == Z64_ITEM_BOMBCHU) { //only have chus
    ...
    if ((dropId == ITEM00_BOMBS_A || dropId == ITEM00_BOMBS_SPECIAL || dropId == ITEM00_BOMBS_B) && z64_file.items[Z64_ITEM_BOMB] == -1) {
        return -1;
    }
    if ((dropId == ITEM00_ARROWS_SMALL || dropId == ITEM00_ARROWS_MEDIUM || dropId == ITEM00_ARROWS_LARGE) && z64_file.items[Z64_ITEM_BOW] == -1) {
        return -1;
    }
    if ((dropId == ITEM00_MAGIC_LARGE || dropId == ITEM00_MAGIC_SMALL) && z64_file.magic_capacity_set == 0) {
        return -1;
    }
    if ((dropId == ITEM00_SEEDS) && z64_file.items[Z64_ITEM_SLINGSHOT] == -1) {
        return -1;
    }
