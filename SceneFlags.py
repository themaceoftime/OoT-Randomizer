...
def get_scene_flag_table(world):
    ...
    for i in range(0, 101):
        ...
        for location in world.get_locations():
            if location.type in ['Freestanding', 'ActorOverride', 'RupeeTower', 'Crate', 'SmallCrate', 'Pot', 'FlyingPot', 'Beehive', 'SilverRupee'] and location.scene == i:
                if (
                    location.type in ["Freestanding", "ActorOverride", "SilverRupee"]
